...
$.fn.fullCalendar = function(options) {
	...
	this.each(function() {
		...
		if (sections) {
			header = $("<table class='fc-header width-100'/>")
				.append($("<tr/>")
...
function Grid(element, options, methods) {
  ...
  {
    var str = '<div class="ui-box-container ui-box-outline ui-draggable" id="uibox_'+formatDateForId(date)+'" style="display: none;">';
    str +=    	'<div class="ui-box-title ui-box-text">';
    str +=				formatDateForTitle(date);
    str +=      	'<span id="uibox_close_'+formatDateForId(date)+'" class="ui-box-close"><a>X</a></span>';
    str +=    	'</div>';
...
/*
  function generateMoreBoxById(idx)
  {
    var str =
           '<div class="ui-box-container ui-box-outline ui-draggable" id="uibox_'+idx+'" style="display: none;">';
    str +=    '<div class="ui-box-title ui-box-text">';
                 str += '<span id="uibox_title_'+idx+'"></span>';
    str +=      '<span id="uibox_close_'+idx+'" class="ui-box-close"><a>X</a></span>';
    str +=    '</div>';
    str +=    '<div id="uibox_content_'+idx+'" class="ui-box-content"></div>';
    str += '</div>';
    return str;
  }
*/
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		if (triggerRes === false) {
			...
			if (event._id === modifiedEventId) {
				...
				bindSegHandlers(event, eventElement, seg); //GAiello - this call was missing for more items
			}
	...
	$moreElements.each( function(idx) {
		...
		$('#uibox_content_'+ id).append($("." + seg_id).css({ left: 0, top:0, right:0, position: "relative"}).show() );
		$('#uibox_close_'+ id).bind("click", function(event) { $('#uibox_'+ id).hide(); return false; });
		var $box = $('#uibox_' + id);
