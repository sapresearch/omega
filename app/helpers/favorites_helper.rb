	module FavoritesHelper
	  def link_to_favorite(favorite)
	    link_to(favorite.item_text, tenant_polymorphic_path(favorite.item))
	  end
	...
	#    link_to('Add to Favorites', tenant_favorite_path(model))
	#  end
	#
	#  def remove_from_favorites(model)
	#    link_to('Remove from Favorites', tenant_favorite_path(model))
	#  end
	
	  def add_to_favorites(model)
	    om_button(:plain => true, :icon => 'favorite-add') do
	      link_to('Add as Favorite', tenant_favorite_path(model), :remote => true,
	  ...
	  def remove_from_favorites(model)
	    ...
	    om_button(:plain => true, :icon => 'favorite-remove') do
	      link_to('Remove from Favorites', tenant_favorite_path(model), :remote => true,
	  ...
	  def tenant_favorite_path(model)
