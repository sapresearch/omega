	...
	#  require_permission Contact::PERM_ADMIN, :only => [:new, :edit, :create, :update, :destroy]
	  ...
	  def index
	    ...
	    respond_with('tenant', @contact_skills)
	  end
	  ...
	  def suggest
	    ...
	    respond_with('tenant', @contact_skills) do |format|
	      format.json do
	        begin
	  ...
	  def show
	    respond_with('tenant', @contact_skill)
	  end
	  ...
	  def create
	    respond_with('tenant', @contact_skill = Contact::Skill.create(params[:contact_skill]))
	  end
