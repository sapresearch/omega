	...
	#  require_permission Contact::PERM_ADMIN, :only => [:new, :edit, :create, :update, :destroy]
	  ...
	  def index
	    ...
	    respond_with('tenant', @contact_groups)
	  end
	
	  def show
	    respond_with('tenant', @contact_group)
	  end
	  ...
	  def create
	    respond_with('tenant', @contact_group = Contact::Group.create(params[:contact_group]))
	  end
	  ...
	  def assign
	    ...
	    respond_with('tenant', @group_position)
	  end
	  ...
	  def remove
	    ...
	    respond_with('tenant', @group_positions)
	  end
