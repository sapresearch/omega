...
var defaults = {
	...
	buttonText: {
		prev: '<img src="../../images/left_arrow.png" title="Previous" border="0">',
		next: '<img src="../../images/right_arrow.png" title="Next" border="0">',/*&nbsp;&#9658;&nbsp;*/
		prevYear: '&nbsp;&lt;&lt;&nbsp;',
		nextYear: '&nbsp;&gt;&gt;&nbsp;',
		today: '<img src="../../images/to_day.png" title="Today" border="0">',
		month: '<img src="../../images/month.png" title="Month" border="0">',
		week: '<img src="../../images/week.png" title="Week" border="0">',
		day: '<img src="../../images/day.png" title="Day" border="0">'
	},
	...
	this.each(function() {
		...
		function render(inc) {
			if (elementVisible()) {
				...
				// coloring slots for working hours in day view				
				/*if(view.name == 'agendaDay'){					
					var  x = (view.start).getDay();
					// working hours 8 tp 5:30
					for(var j=16 ; j<35 ;j++){
						//$("div.fc-rowday"+ j +"").attr("class" ,"fc-rowcol fc-rowday"+ j +" custom-"+ x);
						$("div.fc-rowday"+ j +"").attr("class" ,"fc-rowcol fc-rowday"+ j +" custom");
					}					
				}	*/			
				// end
...
function Grid(element, options, methods) {
	...
	function generateMoreBoxByDate(date)
	{
		...
		str += '<div id="uibox_close_'+formatDateForId(date)+'" class="ui-box-close"><a id="xid">X</a></div>';
		str += '</div>';
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					...
					var isHoliday1 = 0;
					for (var h=0; h<options.holidays.length; h++)
					{
						var hdt = new Date(options.holidays[h]);				
						if (+d == +hdt)
						{
							s += "<td class='fc-" +
								dayIDs[d.getDay()] + ' ' + // needs to be first
								...
								(j==dit ? ' fc-leftmost' : '') +
								(rowCnt>1 && d.getMonth() != month ? ' fc-other-month' : '') +
								' '+tm + '-state-holiday' + "' style='height: 70px;'>" +
								(showNumbers ? "<div class='fc-day-number'>" + d.getDate() + "</div>" : '') +
								"<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div>"+generateMoreBoxByDate(d)+"</td>";
							isHoliday1 = 1;	
							break;	
						}
					} // end for
					...
					if (isHoliday1 == 0)
					{
						s += "<td class='fc-" +
							...
							(+d == +today ?
							' fc-today '+tm+'-state-highlight' :
							' fc-not-today') + "' style='height: 70px;'>" +
							(showNumbers ? "<div class='fc-day-number'>" + d.getDate() + "</div>" : '') +
							"<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div>"+generateMoreBoxByDate(d)+"</td>";
					}
					/* holidays */	
				...
				for (i=prevRowCnt; i<rowCnt; i++) {
					...
					for (j=0; j<colCnt; j++) {
						s += "<td class='fc-" +
							...
							(j==dit ? ' fc-leftmost' : '') + "' style='height: 70px;'>" +
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		/* more */
		/*html +=
			"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px; border-color: rgb("+event.aColor+")'>" +
				"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + " style='background-color : rgb("+event.aColor+"); border-color: rgb("+event.aColor+")'>" +
		...
		if (seg.isMore == undefined)
		{
			var unassign1 = "";
			if(event.aEmpId == 0)
			  unassign1 = " <img src='../../images/assign.png' border='0' title='Unassigned Appointment' alt='Unassigned Appointment'>";
			
			html +=
			"<div class='" + className + event.className.join(' ') + ((seg.isHidden)?" seg_"+formatDateForId(seg.start):"")+ "' style='position:absolute;z-index:8;left:"+left+"px;"+((seg.isHidden)?"display:none;":"")+" border-color: rgb("+event.aColor+")'>" +
			"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + " style='background-color : rgb("+event.aColor+"); border-color: rgb("+event.aColor+")'>" +
			(!event.allDay && seg.isStart ?
			"<span class='fc-event-time'>" +
			htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options)) +
			"</span>"
			:'') +
			"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" + unassign1 +
			"</a>" +
			((event.editable || event.editable == undefined && options.editable) && !options.disableResizing && $.fn.resizable ?
			"<div class='ui-resizable-handle ui-resizable-" + (rtl ? 'w' : 'e') + "'></div>"
			: '') +
		...
		{
			html +=
			"<div id='seg_"+formatDateForId(seg.start)+"' class='" + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px;"+((seg.isHidden)?"display:none;":"")+"' border-color: rgb("+event.aColor+")>" +
			"<a style='background-color : rgb("+event.aColor+"); border-color: rgb("+event.aColor+")'>" +
			(!event.allDay && seg.isStart ?
			"<span class='fc-event-time'>" +
			htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options)) +
			"</span>"
	...
	for (i=0; i<newcnt; i++) {
		...
		/*if(seg.isMore)
		{
			eventElement[0].style.width = '245px';
		}
		else
		{*/
			if (eventElement = seg.element) {
	...
	for (i=0, rowI=0; rowI<rowCnt; rowI++) {
		...
		while (i<newcnt && (seg = segs[i]).row == rowI) {
			//alert(seg.level +" = "+ levelI);
			if (seg.level != levelI) {				
				top += levelHeight;
				levelHeight = 0;
				levelI++;
	...
	/* more */
	var $moreElements = $('.more');
	$moreElements.each( function(idx) {
		var seg_id = $(this).attr('id');
		var id = seg_id.replace("seg_", "");
		$('#uibox_content_'+ id).html('');
		$('#uibox_content_'+ id).append($("." + seg_id).css({ left: 1, top:0, right:0, width:'245px', position: "relative"}).show() );
		$('#uibox_close_'+ id).bind("click", function(event) { isClick = 1; $('#uibox_'+ id).hide(); });
				
		//$('#uibox_content_'+ id).bind("click", function(event) { eventEdit(event); });
		
		var $box = $('#uibox_' + id);
		$box.css("top", $box.parent().position().top);
		//$box.draggable({snap: true, handle: $('#uibox_' + id + ' div:first'), cursor: 'move', scroll: false });
		
		/**/
		//$(this).bind("click", function(event) { $('.ui-box-container').css('display', 'none'); $('#uibox_'+ id).show(); });
		$(this).bind("click", function(event) { 
			
			$("div[id^='uibox_" + id.substring(0,4) + "']").each(function() {	//GAiello- hack to hide all open MORE divs
				$(this).hide();
			});
			$('#uibox_'+ id).show(); 
			return false;
...
function Agenda(element, options, methods) {
	...
	function renderAgenda(c, colFormat) {
		...
		if (!head) { // first time rendering, build from scratch
			...
			for (i=0; d < maxd; i++) {
				...
				if(view.name == 'agendaWeek') {	
					for (x =0; x< colCnt ;x++) {
						...
						if(hmID2 > '08:00' && hmID2 < '18:00' )  {							
								s +="<div  class='fc-rowcol fc-rowdayW"+ i + x +" custom'  style='float: left; position: relative; '>&nbsp;</div>";
						}
						else {
							s +="<div  class='fc-rowcol fc-rowdayW"+ i + x +"' style='float: left; position: relative; height:100%; '> &nbsp; </div>";							
						}					

					} // end of for loop 
				} // end of if agendaweek	
				else { // For agenda Day 	
					s +="<div  class='fc-rowcol fc-rowday"+ i +"' style='float: left; position: relative; height:100%; z-index: 7;'>&nbsp;</div>";							
			}		
			s +="</td></tr>";	*/		
			// end code for working hours		
						
				// working hours
				/*y = formatDate(d, options.axisFormat);   // newcode for Timeslot Color ------------------------------------
				minutes = d.getMinutes();    
				if(y!= '10am' && y!= '10:30am' && y!= '11am' && y!= '11:30am' && y!= '12pm' && y!= '12:30pm'
				  && y!= '2pm' && y!= '2:30pm' && y!= '3pm' && y!= '3:30pm' && y!= '4pm' && y!= '4:30pm' 
				  && y!= '5pm' && y!= '5:30pm' && y!= '6pm' && y!= '6:30pm'){
				s += "<tr class='" +
					(!i ? 'fc-first' : (!minutes ? '' : 'fc-minor')) +
					"'><th class='fc-axis fc-leftmost " + tm + "-state-default'>" +
					((!slotNormal || !minutes) ? formatDate(d, options.axisFormat) : '&nbsp;') + 
					"</th><td class='fc-slot" + i + ' ' +
						tm + "-state-default'><div style='position:relative;z-index:1'>&nbsp;</div></td></tr>";
				}else{
					s += "<tr class='" +
					(!i ? 'fc-first' : (!minutes ? '' : 'fc-minor')) +
					"'><th class='fc-axis fc-leftmost " + tm + "-state-default'>" +
					((!slotNormal || !minutes) ? formatDate(d, options.axisFormat) : '&nbsp;') + 
					"</th><td class='fc-slot" + i + ' ' +
						tm + "-state-default' style='background-color:#FFFFFF; opacity:0.5; filter: alpha(opacity=30);'><div style='position:relative'>&nbsp;</div></td></tr>";
				}*/		
			...
			//alert(+d +" = "+ +today+"\n"+d +" = "+today);
			//alert(new Date(options.holidays[0]));
/*			var hdt = new Array();
			for (var h=0; h<options.holidays.length; h++)
			{
				hdt[h] = new Date(options.holidays[h]);
			}
*/			//alert('agenda 1');
			for (i=0; i<colCnt; i++) {
				...
				for (var h=0; h<options.holidays.length; h++)
				{
					...
					if (+d == +hdt)
					{
						s += "<td class='fc-" +
							dayIDs[d.getDay()] + ' ' + // needs to be first
							tm + '-state-default ' +
							(!i ? 'fc-leftmost ' : '') + tm + '-state-holiday' + 							
							"'><div class='fc-day-content'><div>&nbsp;</div></div></td>";
						isHoliday = 1;	
						break;	
					}
...
/* working hours*/
			/*s1 = "<div class='fc-agenda-bg' style='position:absolute;z-index:1'>" +
					"<table style='width:100%;height:100%'><tr class='fc-first'>";
					
				for (i=0; i<colCnt; i++) {
					var wclass = 'fc-not-today';	
					if (i == 1)
						wclass = 'fc-state-working ';
						
					s1 += "<td class='fc-" +
								dayIDs[d.getDay()] + ' ' + // needs to be first
								tm + '-state-default ' +
								(!i ? 'fc-leftmost ' : '') + wclass +  							
								"'><div class='fc-day-content'><div>&nbsp;</div></div></td>";	
					addDays(d, dis);
					if (nwe) {
						skipWeekend(d, dis);
				...
				s1 += "</tr></table></div>";
				bg = $(s1).appendTo(element);*/
				//var bodyDiv = element.find('fc-agenda-body');				
/* working hours*/
	...
	function setWidth(width) {
		...
		// working hours
		//var cust = body.find('.custom');
		//cust.width(colWidth-1);
		// end
	}
		...
		for (i=0; i<segCnt; i++) {
			...
			if (eventElement = seg.element) {
				...
				if (seg.titleTop !== undefined && height - seg.titleTop < 10) {
					...
					{
						eTitle = event.title.substr(0,12);						
						eTitle += "...";
					}
					eventElement.find('span.fc-event-time')
						.text(formatDate(event.start, view.option('timeFormat')) + ' - ' + eTitle);
					
					if(event.aEmpId == 0)						
						eventElement.find('span.fc-event-time').append(" <img src='../../images/assign.png' border='0' title='Unassigned Appointment' alt='Unassigned Appointment'>");	
	...
	function slotSegHtml(event, seg, className) {
		var unassign = "";
		if(event.aEmpId == 0)
		  unassign = " <img src='../../images/assign.png' border='0' title='Unassigned Appointment' alt='Unassigned Appointment'>";
		 
		return "<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px; border-color: rgb("+event.aColor+")'>" +
			"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + " style='background-color : rgb("+event.aColor+"); border-color: rgb("+event.aColor+")'>" +
				"<span class='fc-event-bg'></span>" +
				"<span class='fc-event-time' style='background-color : rgb("+event.aColor+"); border-color: rgb("+event.aColor+")'>" + htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'))) + "</span>" +
				"<span class='fc-event-title'>" + htmlEscape(event.title) + unassign + "</span>" +
			"</a>" +
			((event.editable || event.editable === undefined && options.editable) && !options.disableResizing && $.fn.resizable ?
